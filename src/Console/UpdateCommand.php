...
namespace Liplum\SyncProfile\Console;
...
class UpdateCommand extends Command
{
   ...
   * {@inheritdoc}
   */
  protected $signature = 'liplum:sync-profile:update';
  ...
  public function handle()
  {
    ...
    $apiToken = $this->settings->get('liplum-sync-profile.api_token');
    $accountType = $this->settings->get('liplum-sync-profile.account_type');
    $login = strtolower($this->settings->get('liplum-sync-profile.login'));
    $groupId = $this->settings->get('liplum-sync-profile.group_id');
    ...
    $usersManaging = collect(json_decode($this->settings->get('liplum-sync-profile.users', '[]')));
  ...
  {
    $this->settings->set('liplum-sync-profile.users', $users->values()->unique()->toJson());
  }
}
